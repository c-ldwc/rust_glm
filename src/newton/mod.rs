...
pub mod step {
    ...
    pub fn optim(
        ...
        let coef_dim = X.shape().1;
        let mut coef = DVector::zeros(coef_dim);
        ...
        for _i in 0..max_iter{
            ...
            while !wolfe && wolfe_checks < 100{
                wolfe = check_wolfe(&dir, &a, &c_1, &c_2, grad, &coef);
                a = rho * a;
    ...
    fn check_wolfe(dir: &DVector<f64>, a:&f64, c_1:&f64, c_2:&f64, grad: fn(&DVector<f64>) -> DVector<f64>, x: &DVector<f64>) -> bool {
        let nab = grad(&x);
        let ar = armijo(&dir, &a, c_1, &nab, &x);
        let cur = curve(&dir, &a, c_2, grad, &x);
        cur & ar
    }
}
